...
@JsonDeserialize(builder = FeedbackExperimentItem.Builder::class)
@NoAutoDetect
...
private constructor(
    ...
    class Builder {
        ...
        internal fun from(feedbackExperimentItem: FeedbackExperimentItem) = apply {
            this.id = feedbackExperimentItem.id
            this.scores = feedbackExperimentItem.scores
            this.expected = feedbackExperimentItem.expected
            this.comment = feedbackExperimentItem.comment
            this.metadata = feedbackExperimentItem.metadata
            this.source = feedbackExperimentItem.source
            this.tags = feedbackExperimentItem.tags
            additionalProperties(feedbackExperimentItem.additionalProperties)
        }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return /* spotless:off */ other is FeedbackExperimentItem && id == other.id && scores == other.scores && expected == other.expected && comment == other.comment && metadata == other.metadata && source == other.source && tags == other.tags && additionalProperties == other.additionalProperties /* spotless:on */
    }

    /* spotless:off */
    private val hashCode: Int by lazy { Objects.hash(id, scores, expected, comment, metadata, source, tags, additionalProperties) }
    /* spotless:on */

    override fun hashCode(): Int = hashCode

    override fun toString() =
        "FeedbackExperimentItem{id=$id, scores=$scores, expected=$expected, comment=$comment, metadata=$metadata, source=$source, tags=$tags, additionalProperties=$additionalProperties}"
}
