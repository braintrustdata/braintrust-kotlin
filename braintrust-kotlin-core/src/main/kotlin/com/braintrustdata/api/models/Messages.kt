...
private constructor(
    ...
    companion object {
        ...
        fun ofSystem(system: System) = Messages(system = system)

        fun ofUser(user: User) = Messages(user = user)

        fun ofAssistant(assistant: Assistant) = Messages(assistant = assistant)

        fun ofTool(tool: Tool) = Messages(tool = tool)

        fun ofFunction(function: Function) = Messages(function = function)

        fun ofFallback(fallback: Fallback) = Messages(fallback = fallback)
    }
