...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String = asString() ?: throw BraintrustInvalidDataException("Value is not a string")
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw BraintrustInvalidDataException("`$name` is not set")
            is JsonNull -> throw BraintrustInvalidDataException("`$name` is null")
            else -> throw BraintrustInvalidDataException("`$name` is invalid, received $this")
        }
